...
const LEGACY_APPROVAL_EVENT_TYPE_MAP: Record<string, string> = {
	approval: 'approval.instance.status_changed_v4',
	approval_instance: 'approval.instance.status_changed_v4',
	approval_task: 'approval.task.status_changed_v4',
};
...
function normalizeEventType(eventType?: string) {
	...
	return LEGACY_APPROVAL_EVENT_TYPE_MAP[eventType] || eventType;
}
...
export default class RequestHandle {
	...
	parse(data: any) {
		...
		if ('schema' in targetData) {
			...
			return {
				[CEventType]: normalizeEventType(targetData?.header?.event_type),
				...rest,
				...header,
				...event,
			};
		...
		return {
			[CEventType]: normalizeEventType(targetData?.event?.type),
			...event,
			...rest,
		};
	}
