...
const SpreadsheetSheetsAddOperate: ResourceOperations = {
	...
	async call(this: IExecuteFunctions, index: number): Promise<IDataObject> {
		...
		const body: IDataObject = { requests: [{ copySheet: { source: { sheetId }, destination: { title } } }] };
		const requestOptions: any = { method: 'POST', url: `/open-apis/sheets/v2/spreadsheets/${spreadsheetToken}/sheets_batch_update`, body };
