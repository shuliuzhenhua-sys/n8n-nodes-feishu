...
const SpreadsheetCellsFindOperate: ResourceOperations = {
	...
	async call(this: IExecuteFunctions, index: number): Promise<IDataObject> {
		...
		const body: IDataObject = { find_condition: { range, match_case: matchCase, match_entire_cell: matchEntireCell, search_by_regex: searchByRegex, include_formulas: includeFormulas }, find };
		const requestOptions: IDataObject = { method: 'POST', url: `/open-apis/sheets/v3/spreadsheets/${spreadsheetToken}/sheets/${sheetId}/find`, body };
