...
const WikiSpacesDeleteMemberOperate: ResourceOperations = {
	...
	async call(this: IExecuteFunctions, index: number): Promise<IDataObject> {
		...
		const body: IDataObject = { member_type: memberType, member_role: memberRole };
		if (type) body.type = type;

		const requestOptions: IDataObject = { method: 'DELETE', url: `/open-apis/wiki/v2/spaces/${spaceId}/members/${memberId}`, body };
